...
@section('page_title', 'Surat Tanda Terima Barang')
        ...
        <div class="stat-box stat-total">
            <div class="d-flex justify-content-between align-items-center">
                <div>
                    ...
                    <h4 class="mb-0 fw-bold" style="font-size: 1.25rem;">{{ $grouped->count() }}</h4>
                </div>
        ...
        <div class="stat-box stat-items">
            <div class="d-flex justify-content-between align-items-center">
                <div>
                    ...
                    <h4 class="mb-0 fw-bold" style="font-size: 1.25rem;">{{ $grouped->sum('total_items') }}</h4>
                </div>
        ...
        <div class="stat-box stat-qty">
            <div class="d-flex justify-content-between align-items-center">
                <div>
                    ...
                    <h4 class="mb-0 fw-bold" style="font-size: 1.25rem;">{{ $grouped->sum('total_qty') }}</h4>
                </div>
...
<div class="group-card mb-3 animate-fade-up" style="animation-delay: {{ $index * 0.05 }}s;">
    <div class="group-header d-flex justify-content-between align-items-center">
        <div>
            <div class="d-flex align-items-center gap-2 mb-1">
                <span class="fw-bold" style="font-size: 0.9375rem;">{{ $group['nama_pengambil'] }}</span>
                @if($group['ruangan'])
                <span class="badge-item bg-primary text-white">{{ $group['ruangan']->nama_ruangan }}</span>
                @endif
            </div>
            <div class="d-flex align-items-center gap-3">
                <span class="text-muted" style="font-size: 0.6875rem;">
                    <i class="fas fa-calendar me-1"></i>
                    @if($group['tanggal_keluar'])
                        {{ \Carbon\Carbon::parse($group['tanggal_keluar'])->translatedFormat('d F Y') }}
                    @else
                        -
                    @endif
                </span>
                <span class="text-muted" style="font-size: 0.6875rem;">
                    <i class="fas fa-box me-1"></i>{{ $group['total_items'] }} jenis barang
                </span>
                <span class="text-muted" style="font-size: 0.6875rem;">
                    <i class="fas fa-cubes me-1"></i>Total: {{ $group['total_qty'] }} unit
                </span>
            </div>
        </div>
        <div>
            <form method="GET" action="{{ route('surat-tanda-terima.generate') }}" class="d-inline">
                <input type="hidden" name="nama_pengambil" value="{{ $group['nama_pengambil'] }}">
                <input type="hidden" name="tanggal_keluar" value="{{ $group['tanggal_keluar'] ? \Carbon\Carbon::parse($group['tanggal_keluar'])->format('Y-m-d') : '' }}">
    ...
    <div class="px-2 py-1">
        ...
        <div class="item-row d-flex justify-content-between align-items-center">
            <div class="d-flex align-items-center gap-2">
                <span class="text-muted" style="font-size: 0.6875rem; width: 24px;">{{ $loop->iteration }}</span>
                <span class="fw-medium" style="font-size: 0.8125rem;">{{ $item->barang->nama_barang ?? '-' }}</span>
            </div>
            <div class="d-flex align-items-center gap-3">
                <span class="fw-bold text-warning" style="font-size: 0.8125rem;">{{ $item->jumlah_keluar }}</span>
                <span class="text-muted" style="font-size: 0.6875rem;">{{ $item->barang->satuan ?? '-' }}</span>
            </div>
        </div>
        @endforeach
    </div>
