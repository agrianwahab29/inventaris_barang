...
@section('title', 'Tambah Barang - Aplikasi Inventaris')
            ...
            <div class="card-body">
                <form method="POST" action="{{ route('barang.store') }}">
                    ...
                    <div class="mb-3">
                        ...
                        @enderror
                        <small class="text-muted">Jika barang sudah ada, stok akan ditambahkan</small>
                    </div>
                            ...
                            <div class="input-group">
                                <select name="satuan_select" class="form-select" id="satuanSelect">
                                    ...
                                    @foreach($satuans as $sat)
                                        <option value="{{ $sat }}" {{ old('satuan_select') == $sat ? 'selected' : '' }}>{{ $sat }}</option>
                                    @endforeach
                                    <option value="__custom__">+ Tambah Satuan Baru</option>
                                </select>
                                <input type="text" name="satuan" class="form-control @error('satuan') is-invalid @enderror" id="satuanInput" value="{{ old('satuan') }}" placeholder="Atau ketik satuan" {{ old('satuan') ? '' : 'style=display:none' }}>
                            </div>
                            @error('satuan')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
