...
class BerkasTransaksiDummySeeder extends Seeder
{
    /**
    ...
    public function run()
    {
        $this->command->info('Creating dummy PDF files and database records...');
        
        // Ensure storage directory exists
        Storage::disk('public')->makeDirectory('berkas-transaksi');
        
        // Create a simple dummy PDF content (PDF header + some content)
        $pdfTemplate = $this->createDummyPdfContent();
        
        // Generate 100 dummy berkas
        $count = 100;
        ...
        for ($i = 0; $i < $count; $i++) {
            ...
            $fileName = $this->getRandomFileName($i);
            $filePath = 'berkas-transaksi/' . $uuid . '.pdf';
            
            // Save dummy PDF file
            Storage::disk('public')->put($filePath, $pdfTemplate);
            
            // Create database record
            BerkasTransaksi::factory()->create([
                'user_id' => $user->id,
                'file_path' => $filePath,
                'file_name' => $fileName,
                'file_size' => strlen($pdfTemplate),
        ...
        $this->command->info("✅ Successfully created {$count} dummy PDF files!");
        $this->command->info('📁 Files are stored in: storage/app/public/berkas-transaksi/');
        $this->command->info('🗄️  Database records created in: berkas_transaksis table');
    }
    
    /**
     * Create a simple dummy PDF content
     */
    private function createDummyPdfContent()
    {
        ...
        $pdf .= "<< /Length 55 >>\n";
        $pdf .= "stream\n";
        $pdf .= "BT /F1 12 Tf 100 700 Td (Dummy PDF Document) Tj ET\n";
        $pdf .= "endstream\n";
        $pdf .= "endobj\n";
        $pdf .= "xref\n";
        $pdf .= "0 5\n";
    ...
    /**
     * Get random file name from list
     */
    private function getRandomFileName($index)
    {
        $fileNames = [
        ...
        return $fileNames[$index % count($fileNames)];
    }
}
